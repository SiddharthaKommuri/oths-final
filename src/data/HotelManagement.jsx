...
import { createBooking, getBookingsByType } from "../../services/BookingServiceSid"; // Import getBookingsByType
...
const HotelManagement = () => {
  ...
  // State for booking counts per hotel
  const [hotelBookingCounts, setHotelBookingCounts] = useState({}); // New state to store hotelId -> count map
  ...
  useEffect(() => {
    const fetchHotels = async () => {
      ...
      try {
        ...
        if (filters.search && filters.location === "all") {
          // If only search is used, and location filter is 'all', try search by location API
          // Note: Your backend search API is only by location, not general search by name/other fields.
          // For a general search, you'd need a backend endpoint that searches across multiple fields.
          // For now, if search is active, we'll try to use searchHotelsByLocation if location is 'all'.
          // Otherwise, we'll fetch all and filter locally.
          fetchedData = await getAllHotels(token); // Fetch all and filter locally for general search
        } else if (filters.location !== "all") {
          // If a specific location is selected, use the backend search API
          fetchedData = await searchHotelsByLocation(filters.location, token);
        } else {
          // Default: Fetch all hotels
    ...
    fetchHotels();
  }, [token, filters.location]); // Re-fetch when token or location filter changes

  // Effect to fetch and calculate booking counts
  useEffect(() => {
    const fetchBookingCounts = async () => {
      if (!token || hotels.length === 0) {
        // Only fetch if token is available and hotels data has been loaded
        return;
      }

      try {
        // Fetch all bookings of type 'hotel'.
        // NOTE: The backend's getBookingsByType is paginated.
        // For simplicity, we're requesting a large page size (1000) to get most bookings.
        // For a very large number of bookings, you would need to implement pagination
        // iteration here or a dedicated backend endpoint for hotel booking counts.
        ...
        // Optionally set an error state specific to booking counts if needed
      }
    };

    fetchBookingCounts();
  }, [token, hotels]); // Re-run when token or hotels data changes

  // Effect to filter hotels based on filters state
  useEffect(() => {
    filterHotels();
  ...
  const getBookingCount = (hotelId) => {
    return hotelBookingCounts[hotelId] || 0; // Return count from state, default to 0 if not found
  };

  const getHotelStatus = (hotel) => {
    if (hotel.roomsAvailable === 0) return "Full";
    if (hotel.status === "Inactive" || hotel.status === "inactive") // Assuming 'status' field might exist
      return "Inactive";
    return "Available";
  };
  ...
  const handleConfirmBooking = async () => {
    ...
    // For simplicity, generate a mock payment ID. In a real app, this comes from a payment gateway.
    const mockPaymentId = Math.floor(Math.random() * 1000000000) + 1;

    // Ensure user.userId is available from AuthContext
    const userIdFromAuth = user.userId || user.id; // Use 'id' as fallback if 'userId' isn't directly on user object
    ...
    try {
      ...
      // After successful booking, re-fetch hotel data to update available rooms and booking counts
      // This will trigger the useEffects to update the table
      // You might also want to update the local 'hotels' state more efficiently if only roomsAvailable changed
      // For now, a full re-fetch is simpler.
      // Call fetchHotels directly or trigger its dependency (e.g., update a state that it depends on)
      // For simplicity, let's just re-fetch all hotels.
      // Note: This needs to be done carefully to avoid infinite loops if fetchHotels depends on 'hotels'
      // The current setup should work as fetchHotels depends on 'token' and 'filters.location'.
      // A better way might be to pass a refresh function down or dispatch an event.
      // For now, we'll rely on the next fetch cycle or a manual trigger if needed.
      // A simple way to trigger re-fetch is to change a state that fetchHotels depends on.
      // Or, if you want to update roomsAvailable immediately:
      setHotels(prevHotels => prevHotels.map(h => 
        h.hotelId === selectedHotel.hotelId ? { ...h, roomsAvailable: h.roomsAvailable - 1 } : h
      ));
      // Also update booking counts locally
